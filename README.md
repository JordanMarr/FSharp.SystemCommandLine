...
The named record version looks like this:

```F#
type UnzipArgs = {
    ZipFile: FileInfo
    OutputDir: DirectoryInfo option
}

let unzip (args: UnzipArgs) =
    let outputDir = defaultArg args.OutputDir args.ZipFile.Directory
    printfn $"Unzipping {args.ZipFile.Name} to {outputDir.FullName}..."
...
let main argv =
    rootCommand argv {
        ...
        input {
            let! zipFile = argument "zipfile" |> desc "The file to unzip" |> validateFileExists
            and! outputDir = optionMaybe "--output" |> alias "-o" |> desc "The output directory"
            return { ZipFile = zipFile; OutputDir = outputDir }
        }
        setAction unzip
    }
```
